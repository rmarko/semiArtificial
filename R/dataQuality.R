...
dataSimilarity <- function(data1, data2, dropDiscrete=NA) {
  ...
  if (ncol(data1Num) > 0) {
    ...
    s1[5,] <- apply(data1Num, 2, function(x) mc(jitter(x, factor=1e-7), na.rm=TRUE)) # medcouple MC
    med1 <- apply(data1Num, 2, median, na.rm=TRUE)
    for (i in 1:ncol(data1Num)) {
	  leftData <- data1Num[!is.na(data1Num[,i]) & data1Num[,i] < med1[i], i]
	  if (length(leftData) == 0)
		  s1[6,i] <- 0
	  else
         s1[6,i] <-  - mc(jitter(leftData, factor=1e-7), na.rm=TRUE) # left MC, LMC
	 
	 rightData <- data1Num[!is.na(data1Num[,i]) & data1Num[,i] > med1[i], i]
	 if (length(rightData) == 0)
		 s1[7,i] <- 0
	 else
		 s1[7,i] <-  mc(jitter(rightData, factor=1e-7), na.rm=TRUE) # right MC, RMC
    }
    ...
    s2[5,] <- apply(data2Num, 2, function(x) mc(jitter(x, factor=1e-7), na.rm=TRUE)) # medcouple MC
    med2 <- apply(data2Num, 2, median, na.rm=TRUE)
    for (i in 1:ncol(data2Num)) {
		leftData <- data2Num[!is.na(data2Num[,i]) & data2Num[,i] < med2[i], i]
		if (length(leftData) == 0)
			s2[6,i] <- 0
		else
			s2[6,i] <-  - mc(jitter(leftData, factor=1e-7), na.rm=TRUE) # left MC, LMC
		
		rightData <- data2Num[!is.na(data2Num[,i]) & data2Num[,i] > med2[i], i]
		if (length(rightData) == 0)
			s2[7,i] <- 0
		else
			s2[7,i] <-  mc(jitter(rightData, factor=1e-7), na.rm=TRUE) # right MC, RMC
    }
    ...
    s1Norm[5,] <- apply(data1NumNorm, 2, function(x) mc(jitter(x, factor=1e-7), na.rm=TRUE)) # medcouple MC
    ...
    for (i in 1:ncol(data1NumNorm)) {
		...
		if (length(leftData) == 0)
			...
			s1Norm[6,i] <-  - mc(jitter(leftData, factor=1e-7), na.rm=TRUE) # left MC, LMC
		
		rightData <- data1NumNorm[!is.na(data1NumNorm[,i]) & data1NumNorm[,i] > med1n[i], i]
		if (length(rightData) == 0)
			s1Norm[7,i] <- 0
		else
			s1Norm[7,i] <-  mc(jitter(rightData, factor=1e-7), na.rm=TRUE) # right MC, RMC
    }
    ...
    s2Norm[5,] <- apply(data2NumNorm, 2, function(x) mc(jitter(x, factor=1e-7), na.rm=TRUE)) # medcuple MC
    ...
    for (i in 1:ncol(data2NumNorm)) {
		...
		if (length(leftData) == 0)
			...
			s2Norm[6,i] <-  - mc(jitter(leftData, factor=1e-7), na.rm=TRUE) # left MC, LMC
		
		rightData <- data2NumNorm[!is.na(data2NumNorm[,i]) & data2NumNorm[,i] > med2n[i], i]
		if (length(rightData) == 0)
			s2Norm[7,i] <- 0
		else
			s2Norm[7,i] <-  mc(jitter(rightData, factor=1e-7), na.rm=TRUE) # right MC, RMC
   }
